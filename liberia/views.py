...
class TableTemplateView(generic.TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['latest_qs'] = SitRep.objects.latest('formatted_date')
        latest = datetime.strftime(context['latest_qs'].formatted_date, "%j")
        nums = [int(latest)]
        num=int(latest)
        ...
        #Excluding National
        no_nat_list = []
        for i in context['latest_qs'].locationsitrep_set.values('location__name', 'cases_cum', 'total_deaths_all', 'hcw_cases_cum', 'hcw_deaths_cum').exclude(location=national):
        ...
        # #Including national
        # inc_nat_list = []
        # for i in context['latest_qs'].locationsitrep_set.values('location__name', 'cases_cum', 'total_deaths_all', 'hcw_cases_cum', 'hcw_deaths_cum'):
        #     daily_deaths = locs_daily_dict[i['location__name']].get('daily_deaths')
        #     daily_cases = locs_daily_dict[i['location__name']].get('daily_cases')
        #     i.setdefault('daily_deaths', daily_deaths)
        #     i.setdefault('daily_cases', daily_cases)
        ...
        # #Just national
        # nat_list = []
        # for i in context['latest_qs'].locationsitrep_set.values('location__name', 'cases_cum', 'total_deaths_all', 'hcw_cases_cum', 'hcw_deaths_cum').filter(location=national):
        #     daily_deaths = locs_daily_dict[i['location__name']].get('daily_deaths')
        #     daily_cases = locs_daily_dict[i['location__name']].get('daily_cases')
        #     i.setdefault('daily_deaths', daily_deaths)
        #     i.setdefault('daily_cases', daily_cases)
